...
> **来源**：[../../prd/00-总览与设计原则.md](../../prd/00-总览与设计原则.md) · [../../prd/01-课程目录骨架.md](../../prd/01-课程目录骨架.md) · [../../prd/02-知识点与题型内容模板.md](../../prd/02-知识点与题型内容模板.md) · [../../prd/03-数据库schema.md](../../prd/03-数据库schema.md)
...
- **可见性**：用户端**永远不查待发布内容**——通过 RLS / API 层强制 `is_published=true`，目录类同时校验父级已发布
...
- ✅ 14 个核心表是否覆盖 prd/03 全部对象
- ✅ KP 内容字段（按 7 类差异）是否完整可生成
- ✅ 题型 12 类的 `payload` 结构是否能渲染所有用户端题型
- ✅ 导入批次表是否能支撑 [prd/06 §6.3 流程 B](../../prd/06-关键业务流程.md) 的批量导入、校验与追溯
