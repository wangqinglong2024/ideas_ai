...
**对应需求中的操作**：[prd/05 §5.6 节末小测](../../prd/05-用户端模块设计.md) + [prd/06 §6.7 流程 F](../../prd/06-关键业务流程.md)
...
- 抽题：只从已发布题目中抽取；下架题目不进入新题包（参见 [prd/02 §2.5 组卷](../../prd/02-知识点与题型内容模板.md) 默认规则）
...
```json
{
  ...
  "data": {
    ...
    "questions": [
      ...
      /* ... 默认 6 题；若有效已发布题不足，可少于 6 题并按有效题数动态均分 */
    ],
    "pass_score": 80
  }
}
...
| 409 | `COURSE_EXAM_EFFECTIVE_QUESTION_EMPTY` | 当前有效已发布题为 0 |
| 404 | 40400 | 节不存在 |
...
**对应需求中的操作**：[prd/05 §5.5 节内练习 / §5.6 节末小测 / §5.8 SRS 复习](../../prd/05-用户端模块设计.md) + [prd/06 §6.7](../../prd/06-关键业务流程.md)
...
**对应需求中的操作**：[prd/05 §5.6](../../prd/05-用户端模块设计.md) + [prd/06 §6.7](../../prd/06-关键业务流程.md)
...
- 总分按当次 `quiz_id` snapshot 中有效题计算，满分固定 100；单题分 = `100 / effective_question_count`；通过 = `score >= pass_score`
...
**对应需求中的操作**：[prd/05 §5.8 SRS 复习](../../prd/05-用户端模块设计.md) + [prd/06 §6.8 流程 G](../../prd/06-关键业务流程.md)
...
**对应需求中的操作**：[prd/05 §5.8 错题本](../../prd/05-用户端模块设计.md)
