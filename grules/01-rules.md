...
### 1. 上下文优先原则（Context-First）

> **AI 输出质量 = f(上下文质量)**。上下文不够就不动手。

- 开始任何任务前，必须完成 `task-workflow.md` §一「上下文加载协议」的 6 项检查
- 禁止在上下文不完整时编码——宁可多问一个问题，也不要猜错一个需求
- 每次搜索代码库至少使用 3 个不同角度的关键词，确保不遗漏现有实现

### 2. 先查后建原则（Look Before You Leap）
...
### 3. 两阶段法则（Plan → Execute）

> **计划和执行必须严格分离，禁止边想边写。**

- **Phase 1 计划**：分析需求、搜索代码、设计方案、拆解任务。此阶段禁止写任何业务代码
- **Phase 2 执行**：按计划逐任务实现。此阶段禁止修改计划范围（需调整时先暂停讨论）
- 复杂任务（> 5 文件或复杂度 L）必须先创建 Story File（见 `task-workflow.md` §三）

### 4. 增量验证原则（Verify As You Go）

> **不要等全部写完再测，写一步验一步。**

- 每写完一个函数/组件 → 立即编译检查
- 每写完一个任务 → Docker 构建 + 验收标准逐条验证
- 每修复一个错误 → 重新走完整验证流程
- 验证顺序：TypeScript 编译 → Docker 构建 → 功能验证 → 自检清单
...
> AI 在完成每个任务后，自动执行此检查表。详见 `task-workflow.md` §五.2。
