...
FastAPI 底层是 ASGI。为了真正发挥高并发优势，从请求到响应的每一行代码必须是**全链路异步**的。核心原则：

1. **全异步 I/O**：所有路由 `async def`、数据库交互使用异步驱动（`asyncpg` / Supabase Async Client）、第三方调用使用 `httpx`。禁止使用阻塞库（`requests`、同步 DB 驱动）。
2. **JWT 本地无状态验签**：使用 `JWT_SECRET` 在内存中毫秒级验签，绝不每次 HTTP 请求 Supabase 去查 Token。配合 `Depends(get_current_user)` 依赖注入实现零信任鉴权。
...
**连接池 & Worker 配置**：
- Uvicorn 生产部署使用 `gunicorn -k uvicorn.workers.UvicornWorker`，Worker 数量 = `CPU 核数 × 2 + 1`
- 数据库连接池必须配置上限（`asyncpg` pool `min_size=5, max_size=20`），禁止每次请求新建连接
- `httpx.AsyncClient` 必须创建全局单例并复用，禁止在每个请求里 `async with httpx.AsyncClient()`
- Supabase Async Client 同理，在应用启动时初始化一次，通过依赖注入共享

**缓存层**：
- 高频读取、低频变更的数据（配置项、分类列表、热门排行）必须加缓存
- 优先使用 Redis（Docker 部署，接入 `global-data-link` 网络）；简单场景可用 Python `lru_cache` + TTL
...
2. **端到端真实验证**：整合 Browser MCP，在真实浏览器中驱动完整业务链路（前端操作 → Nginx → FastAPI → Supabase → 前端渲染）。
