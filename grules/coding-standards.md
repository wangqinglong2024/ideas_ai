...
| 场景 | 风格 | 示例 |
|------|------|------|
| 文件/目录名 | kebab-case（短横线） | `user-profile.tsx`, `auth-router.py` |
...
| Python 变量/函数 | snake_case | `is_active`, `get_current_user` |
| Python 类 | PascalCase | `UserResponse`, `ChatService` |
| Python 常量 | UPPER_SNAKE_CASE | `JWT_SECRET`, `DB_URL` |
...
class UserInDB(UserResponse):
    hashed_password: str
```
- 每个模型字段必须有 `Field(description="中文说明")`
- 请求模型必须设置字段约束（`min_length`, `max_value`, `regex` 等）
...
```
- 使用全局异常处理器统一捕获，禁止在路由里裸 `try/except`
- 已知业务异常用自定义 `BizError(code, message)` 抛出
- 未知异常由全局处理器兜底，生产环境只返回"服务器内部错误"，不暴露堆栈

### 5. 异步 HTTP 客户端与后台任务
- 调用第三方 API（大模型、支付等）禁止使用阻塞的 `requests` 库，必须使用 `httpx.AsyncClient`
- CPU 密集型任务（报表生成、大图裁剪）禁止在路由函数内同步执行，必须使用 `BackgroundTasks` 或消息队列，立即返回"处理中"
...
### 7. 日志规范
- 使用 `loguru` 或 Python 标准 `logging`，禁止用 `print`
- 日志级别：DEBUG（开发调试）、INFO（关键业务节点）、WARNING（异常但可恢复）、ERROR（需人工介入）
- 每条日志必须包含：`user_id`（如有）、`request_id`、关键业务参数
...
- **后端**：`requirements.txt` 中所有依赖必须使用精确版本号（`==`），禁止 `>=` 或无版本约束；推荐额外维护 `requirements-dev.txt` 区分开发依赖
- **Docker 镜像**：基础镜像使用固定版本标签（如 `node:20-alpine`、`python:3.12-slim`），禁止使用 `latest`（已在安全规范 §六.5 中强调）
- **依赖更新纪律**：禁止随意 `npm update` / `pip install --upgrade`；升级依赖必须单独一个 commit（type: `chore`），并在提交说明中注明升级原因和影响范围
...
### 1. 注入防护（OWASP A03）
- **SQL 注入**：使用 Supabase SDK 参数化查询或 Pydantic 模型绑定，禁止拼接 SQL 字符串；手写 SQL 必须用 `$1, $2` 占位符
- **XSS**：React 默认转义，但 `dangerouslySetInnerHTML` 必须经 DOMPurify 消毒；用户生成的富文本内容存储前必须清洗
- **命令注入**：禁止 `os.system()` / `subprocess.run(shell=True)`，必须传列表参数
- **路径穿越**：文件操作必须校验路径不包含 `..`，使用 `os.path.abspath()` 规范化后验证前缀
...
- 禁止在任何响应中暴露服务器技术栈信息：Nginx 配置 `server_tokens off;`，FastAPI 响应不带 `X-Powered-By`
- 错误页面（404/500）使用自定义页面，禁止默认的 Nginx/FastAPI 错误页（泄露版本号）

### 8. CORS 跨域严格管控

```python
# FastAPI CORS 配置（仅限生产环境允许的域名）
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
    ...
    allow_headers=["Authorization", "Content-Type", "X-Signature", "X-Timestamp", "X-Nonce", "X-Idempotency-Key"],
)
```
- **绝对禁止** `allow_origins=["*"]` 上线到生产环境
- 开发环境可临时放开，但必须在 `.env` 中通过环境变量控制，不坤在代码里
- Nginx 层也要配置 CORS 保持一致，避免 Nginx 和 FastAPI 响应头冲突
...
### 11. 依赖与部署安全
- 禁止安装来源不明的 npm/pip 包，只用星标 > 1000 的主流库
- Docker 镜像使用固定版本标签，禁止 `latest`
- 生产环境禁止开启 `DEBUG=True`、`--reload`、Swagger UI（`docs_url=None, redoc_url=None`）
- Docker 容器以非 root 用户运行（`USER nonroot`），禁止 `--privileged`
- `.env` 文件权限设为 `600`，仅属主可读写
- 定期检查依赖漏洞：前端 `npm audit`，后端 `pip-audit` 或 `safety check`
...
- [ ] 类型完整（TypeScript 无 `any`，Python 有完整 type hints）

### 安全性
- [ ] 用户输入已校验（前端 Zod + 后端 Pydantic）
- [ ] 有鉴权守卫（需要登录的接口有 Depends）
- [ ] RLS 策略已配置（新表刚创建就开启）
- [ ] 无敏感信息硬编码
- [ ] 敏感字段已加密存储或脱敏输出
- [ ] 涉及金额的接口有签名验证 + 幂等处理
...
- [ ] 关键决策有中文注释解释"为什么"
- [ ] 数据库变更有 migration 文件
- [ ] 三端类型同步（DB Schema → Pydantic → TypeScript）
