...
async def get_user_by_phone(phone: str) -> Optional[dict]:
    """通过手机号查询用户（包含冻结状态）"""
    result = await admin_client.table("users").select("*").eq("phone", phone).maybe_single().execute()
    return result.data


async def get_user_by_id(user_id: str) -> Optional[dict]:
    """通过 user_id 查询用户"""
    result = await admin_client.table("users").select("*").eq("id", user_id).maybe_single().execute()
    return result.data
...
async def create_user(user_id: str, phone: str, invited_by: Optional[str] = None) -> dict:
    ...
    result = await admin_client.table("users").insert(user_data).execute()
    user = result.data[0]

    # 同步创建钱包
    await admin_client.table("wallets").insert({"user_id": user_id}).execute()
...
async def update_wechat_openid(user_id: str, openid: str) -> None:
    """更新微信 openid"""
    await admin_client.table("users").update({"wechat_openid": openid}).eq("id", user_id).execute()


async def set_user_frozen(user_id: str, frozen: bool) -> None:
    """冻结/解封用户"""
    await admin_client.table("users").update({"is_frozen": frozen}).eq("id", user_id).execute()
