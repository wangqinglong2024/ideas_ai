...
export function CoursePage({ route, navigate, loggedIn }: { route: string; navigate: (path: string) => void; loggedIn: boolean }) {
  ...
  return <section className="page stack"><h1>Courses</h1><Card><h2>My learning</h2><p>{loggedIn ? 'Continue Stage 1 Chapter 1, then review your wrong set.' : 'Browse tracks now; login unlocks free trials and progress sync.'}</p></Card><div className="track-grid">{tracks.map((track) => <button key={track} className="track-card glass-porcelain" onClick={() => navigate(`/courses/${track}`)}><strong>{titleCase(track)}</strong><small>12 stages · Stage 1 chapters 1-3 free</small>{activeTrack === track ? <Badge>Open</Badge> : null}</button>)}</div>{activeTrack ? <StageMap track={activeTrack} navigate={navigate} /> : null}</section>;
}

function StageMap({ track, navigate }: { track: string; navigate: (path: string) => void }) {
  return <Card><h2>{titleCase(track)} pathway</h2><div className="stage-grid">{Array.from({ length: 12 }, (_, index) => <button key={index} onClick={() => navigate(`/courses/${track}/stage-${index + 1}`)}><span>{index + 1}</span><small>{index === 0 ? 'free sample' : 'locked or owned'}</small></button>)}</div><DataTable rows={Array.from({ length: 12 }, (_, index) => ({ id: index + 1, chapter: `Chapter ${index + 1}`, state: index < 3 ? 'free' : 'permission required' }))} columns={['chapter', 'state']} /></Card>;
}
