...
router.post('/units/:unitId/test', async (req, res: Response) => {
...
router.put('/assessments/:attemptId/progress', async (req, res: Response) => {
  const { sub } = (req as unknown as AuthRequest).user
...
router.get('/assessments/:attemptId/progress', async (req, res: Response) => {
  const { sub } = (req as unknown as AuthRequest).user
  const { attemptId } = req.params
...
router.post('/assessments/:attemptId/submit-test', async (req, res: Response) => {
  const { sub } = (req as unknown as AuthRequest).user
  const { attemptId } = req.params
