...
> 全平台一期产能规划，便于 PM 派单与产能监控；同时把 SEO/GEO 嵌入策略落到 KP / 题目 / lesson 字段。课程域不另建独立 GEO 文件，GEO 必须随内容字段一起生成。
...
| 文化敏感 | 避免政治、宗教、民族矛盾、领土争议、敏感地名等议题；不出现繁体；不出现台湾地名作为主权表述 |
| 成人内容 | 不生成性内容、色情暗示、性交易、露骨身体描写或擦边对话；课程场景一律保持学习、工作、生活服务向 |
| 东南亚合规 | 越南、泰国、印尼优先合规；例句和翻译保持中性、低刺激，不借历史或文化话题映射现实冲突 |
...
> SEO 字段不单独出包；它们必须进入可导入的 lesson / KP / question 数据字段。
...
> GEO 不是独立内容副本；BLUF、entities、examples、sources 必须和 KP 正文同源生成。
...
`entities` 字段在 schema 中是 `content` 的可选 key，导入脚本会单独抽出去做 GEO 索引（不影响主流程）。实体选择同样受合规约束：优先平台、节日、商品、工具、课程概念等低风险实体；避开现实政治、民族、宗教争议实体。
