...
## User Story
**As a** 后端开发
**I want** 启动时自动创建 `zhiyu` schema 并跑迁移
...
- [ ] Drizzle 配置：
  - `drizzle.config.ts` 指向 `DATABASE_URL=postgres://postgres:root1234@supabase-db:5432/postgres?search_path=zhiyu`
  - 迁移目录：`apps/api/drizzle/migrations/`
- [ ] BE 启动时执行 `pnpm db:migrate`：
  1. `CREATE SCHEMA IF NOT EXISTS zhiyu`
  2. 跑迁移
  3. 失败 → 容器 unhealthy
- [ ] 首个迁移文件：`0001_init_meta.sql` 创建表 `zhiyu._meta(id serial pk, version text, applied_at timestamptz default now())`
...
# 期待：{ "schema": "zhiyu", "_meta_rows": 0, "supabase_kong_ok": true, "auth_users": 0 }
```
...
## DoD
- [ ] 测试方法全通过
- [ ] supabase Studio (`https://supabase.ideas.top`) 内可见 `zhiyu` schema 与 `_meta` 表
- [ ] 重启容器幂等（不重复创建）
