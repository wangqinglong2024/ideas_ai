...
全局背景允许一层低对比动态流光，用于回应产品的“AI 速度感”与用户指定的 Stitch 参考，但必须转译为知语的水墨雾带：慢速横向漂移、低饱和、无离散圆点或粒子。阅读正文、后台表格、答题和游戏主画面不得叠加强动态背景；这些页面只保留页面底层的静态材质或降低不透明度。
