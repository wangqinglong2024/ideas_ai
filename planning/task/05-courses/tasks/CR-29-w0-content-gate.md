...
- §4.1：“W0 首发生产范围（前 3 阶段）不等于免费权限范围。”
- `05-acceptance-criteria.md` §3：“W0 首发生产内容：4 轨道 × 前 3 阶段全部上架；登录免费试学权限：每轨 Stage 1 前 3 章可学。”

## 需求落实

- W0 上线门槛检查脚本：`pnpm verify:courses-w0`。
- 校验项：
  1. 4 轨道存在且 status=`active`。
  2. 每轨 Stage 1-3 status=`published`。
  3. 每轨 Stage 1 Chapter 1-3 `is_free=TRUE` 且 `free_reason='login_trial'`。
  4. 每轨 Stage 1 Chapter 4-12 `is_free=FALSE`。
...
- [ ] dev 模式脚本通过 + 报告生成。
- [ ] 故意删除一个 Stage 1 Chapter 1 的 is_free 后脚本失败。
- [ ] 故意缺失一种语种翻译 → 失败。
